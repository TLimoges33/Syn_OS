...
def analyze_technical_debt() -> Tuple[int, List[str]]:
    """Analyze technical debt markers - only genuine comment-based markers"""
    print("🔍 Analyzing technical debt (TODO/FIXME comments)...")
    
    debt_markers = []
    total_markers = 0
    
    # Search for genuine comment-based technical debt markers only
    # Use precise patterns to avoid false positives from platform names, CVE references, etc.
    patterns = [
        r"//\s*TODO",           # // TODO
        r"//\s*FIXME",          # // FIXME  
        r"//\s*XXX",            # // XXX
        ...
        r"/\*\s*XXX",           # /* XXX
        r"/\*\s*HACK",          # /* HACK
        r"#\s*TODO",            # # TODO (Python)
        r"#\s*FIXME",           # # FIXME (Python)
        r"#\s*XXX",             # # XXX (Python)
        r"#\s*HACK"             # # HACK (Python)
    ]
    
    for pattern in patterns:
        cmd = ["grep", "-r", "-n", "-E", pattern, "src/"]
        code, stdout, stderr = run_command(cmd)

...
impl ContextSwitcher {
    ...
    fn save_current_context(&mut self, context: &mut CpuContext) {
        unsafe {
            ...
            // Save segment registers in batches to avoid register pressure
            // First batch: cs, ss, ds
            asm!(
                "mov {:x}, cs",
                "mov {:x}, ss",
                "mov {:x}, ds",
    ...
    fn load_new_context(&mut self, context: &CpuContext) {
        unsafe {
            // Load segment registers in batches to avoid register pressure
            // First batch: ds, es
            asm!(
                "mov ds, {:x}",
                "mov es, {:x}",
...
pub mod arch {
    ...
    pub unsafe fn switch_to_user_mode(
        ...
        crate::println!(
            "Would switch to user code at 0x{:x} with stack at 0x{:x}",
            user_code,
            user_stack
        );
    }

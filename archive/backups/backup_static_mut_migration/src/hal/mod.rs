...
// Phase 5a: Hardware Acceleration modules
pub mod npu;
pub mod crypto;
pub mod gpu_compute;
pub mod ai_runtime;
...
// Re-export Phase 5a components
pub use npu::*;
pub use crypto::*;
pub use gpu_compute::*;
pub use ai_runtime::*;

/// Hardware capability flags for acceleration
...
impl HardwareCapabilities {
    /// Detect hardware capabilities using CPUID
    pub fn detect() -> Self {
        Self {
            has_aes_ni: true,
            has_sha_ext: true,
...
pub enum HalError {
    ...
    PermissionDenied,
    InvalidOperation,
    // Phase 5a: Hardware Acceleration errors
...
impl fmt::Display for HalError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            HalError::DeviceNotInitialized => write!(f, "Device not initialized"),
            HalError::InvalidInput => write!(f, "Invalid input"),
            HalError::OperationFailed => write!(f, "Operation failed"),
            HalError::ModelNotFound => write!(f, "AI model not found"),
            HalError::ModelLoadFailed => write!(f, "AI model load failed"),
            HalError::InsufficientMemory => write!(f, "Insufficient memory"),
...
impl HardwareAbstractionLayer {
    ...
    pub fn get_hardware_summary(&self) -> HardwareSummary {
        ...
        // Count USB controllers from PCI devices (USB Host Controllers)
        let usb_count = self.count_usb_controllers();
    ...
    /// Count USB controllers from PCI devices
    fn count_usb_controllers(&self) -> u32 {
        let mut count = 0;

        for device in &self.pci_manager.devices {
            // USB controllers are class 0x0C (Serial Bus Controller), subclass 0x03 (USB)
            let class_code = (device.class_code >> 16) & 0xFF;
...
/// Global HAL instance
lazy_static! {
    static ref HARDWARE_ABSTRACTION_LAYER: Mutex<HardwareAbstractionLayer> = {
        let mut hal = HardwareAbstractionLayer::new();
        // Initialize HAL during construction
        let _ = hal.init();
        Mutex::new(hal)
    };
}

/// Initialize the global HAL (for compatibility)
pub fn init_hal() -> Result<(), HalError> {
    // Already initialized via lazy_static
    Ok(())
}

/// Execute a function with access to the HAL
pub fn with_hal<F, R>(f: F) -> R
where
    F: FnOnce(&mut HardwareAbstractionLayer) -> R,
{
    let mut hal = HARDWARE_ABSTRACTION_LAYER.lock();
    f(&mut hal)
}

// Removed obsolete get_hal_ref() - use with_hal() instead

#!/bin/bash
# SynOS v1.0 Complete Deployment Script
...
if [ "$EUID" -ne 0 ]; then
    echo -e "${YELLOW}⚠️  This script needs root privileges for chroot operations${NC}"
...
for binary in "${BINARIES[@]}"; do
    if [ -f "target/debug/$binary" ]; then
        ...
        cp "target/debug/$binary" "$CHROOT/usr/local/bin/"
        chmod +x "$CHROOT/usr/local/bin/$binary"

        # Strip debug symbols to reduce size
        strip "$CHROOT/usr/local/bin/$binary" 2>/dev/null || true
...
if [ -f "target/x86_64-unknown-none/release/kernel" ]; then
    ...
    cp "target/x86_64-unknown-none/release/kernel" "$CHROOT/boot/synos/synos-kernel-1.0"
    chmod 644 "$CHROOT/boot/synos/synos-kernel-1.0"

    # Create kernel info file
    cat > "$CHROOT/boot/synos/kernel-info.txt" << 'EOF'
...
chroot "$CHROOT" pip3 install nats-py --break-system-packages 2>/dev/null || \
chroot "$CHROOT" pip3 install nats-py 2>/dev/null || \
echo -e "${YELLOW}    ⚠ Failed to install nats-py (may already be installed)${NC}"

echo -e "  → Verifying AI packages..."
chroot "$CHROOT" pip3 list | grep -E "torch|onnx|langchain|nats" || true
...
if [ -f "$GRUB_CFG" ]; then
    ...
    sed -i 's/hostname=parrot/hostname=synos/g' "$GRUB_CFG"

    # Replace Parrot references with SynOS
    sed -i 's/Parrot Security/SynOS v1.0/g' "$GRUB_CFG"
    sed -i 's/Parrot OS/SynOS/g' "$GRUB_CFG"
...
menuentry "SynOS Custom Kernel v1.0" --class synos {\
    ...
    if [ -f "assets/branding/grub/synos-grub-16x9.png" ]; then
        mkdir -p "$CHROOT/boot/grub/themes/synos"
        cp "assets/branding/grub/synos-grub-16x9.png" "$CHROOT/boot/grub/themes/synos/background.png"
        cp "assets/branding/grub/synos-grub-4x3.png" "$CHROOT/boot/grub/themes/synos/background-4x3.png"
...
# Copy theme files from assets
if [ -f "assets/branding/plymouth/synos-neural/synos-neural.plymouth" ]; then
    cp "assets/branding/plymouth/synos-neural/synos-neural.plymouth" "$PLYMOUTH_THEME_DIR/"

    # Create improved Plymouth theme
    cat > "$PLYMOUTH_THEME_DIR/synos-neural.plymouth" << 'EOF'
[Plymouth Theme]
Name=SynOS Neural
...
EOF
    ...
    cat > "$PLYMOUTH_THEME_DIR/synos-neural.script" << 'EOF'
...
EOF
    ...
    if [ -f "assets/branding/logos/synos-logo-128.png" ]; then
        cp "assets/branding/logos/synos-logo-128.png" "$PLYMOUTH_THEME_DIR/logo.png"
    fi

    echo -e "${GREEN}✓ Plymouth theme deployed${NC}"
else
    echo -e "${YELLOW}⚠ Plymouth theme source not found, creating basic theme${NC}"
...
[text]
title=SynOS v1.0 - Neural Darwinism AI Framework
black=0x001122
white=0x00ffff
brown=0xffffff
blue=0x0066cc
...
cat > "$SERVICES_DIR/synos-threat-intel.service" << 'EOF'
[Unit]
...
cat > "$SERVICES_DIR/synos-threat-hunting.service" << 'EOF'
[Unit]
...
cat > "$SERVICES_DIR/synos-zt-engine.service" << 'EOF'
[Unit]
...
# Enable services (optional - user can enable manually)
...
mkdir -p "$CHROOT/usr/share/backgrounds/synos"
mkdir -p "$CHROOT/usr/share/pixmaps"

# Copy logos
if [ -d "assets/branding/logos" ]; then
    echo -e "  → Copying SynOS logos..."
    cp assets/branding/logos/*.png "$CHROOT/usr/share/pixmaps/" 2>/dev/null || true
fi

# Create default wallpaper (simple gradient for now)
if [ ! -f "$CHROOT/usr/share/backgrounds/synos/default.png" ]; then
    echo -e "  → Creating default wallpaper..."

    # Create a simple solid color wallpaper as placeholder
    # In production, this would be a proper designed wallpaper
    cat > "$CHROOT/usr/share/backgrounds/synos/synos-neural.svg" << 'EOF'
...
cat > "$CHROOT/etc/dconf/db/local.d/01-synos-defaults" << 'EOF'
...
if chroot "$CHROOT" pip3 show nats-py &>/dev/null; then
    echo -e "  ${GREEN}✓${NC} nats-py installed"
else
    echo -e "  ${YELLOW}⚠${NC} nats-py not installed"
fi

if chroot "$CHROOT" pip3 show torch &>/dev/null; then
    echo -e "  ${GREEN}✓${NC} PyTorch installed"
fi

if chroot "$CHROOT" pip3 show onnxruntime &>/dev/null; then
    echo -e "  ${GREEN}✓${NC} ONNX Runtime installed"
fi

if chroot "$CHROOT" pip3 show langchain &>/dev/null; then

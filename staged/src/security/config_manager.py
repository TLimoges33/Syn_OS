...
class SecureConfigManager:
    ...
    def validate_security_config(self) -> bool:
        ...
        try:
            # Get security config and check for issues
            security_config = self.get_security_config()
            
            # Validate JWT secret strength
            if len(security_config.jwt_secret_key) < 32:
                logger.error("JWT secret key is too weak (< 32 characters)")
                return False
            
            # Validate encryption key
            if len(security_config.encryption_key) < 32:
                logger.error("Encryption key is too weak (< 32 characters)")
                return False
            
            # Validate signing key
            if len(security_config.signing_key) < 32:
                logger.error("Signing key is too weak (< 32 characters)")  
                return False
            
            # Validate timeout settings
            if security_config.session_timeout_minutes < 5:
                logger.warning("Session timeout is very short (< 5 minutes)")
            
            if security_config.jwt_expiration_hours > 168:  # 1 week
                logger.warning("JWT expiration is very long (> 1 week)")

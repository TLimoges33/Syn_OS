...
use x86_64::{
    structures::paging::{Page, PageTableFlags, Size4KiB, MapperAllSizes, PhysFrame},
    VirtAddr, PhysAddr,
};
    ...
    for region in regions.iter() {
        if addr >= region.start_addr && addr < region.end_addr {
            ...
            if region.attributes.verified_access {
                ...
                let verification_result = verification::verify_memory_access(addr.as_u64() as usize, size);

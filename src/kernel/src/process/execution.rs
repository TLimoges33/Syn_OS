...
use alloc::vec::Vec;
use alloc::boxed::Box;
use crate::process::elf_loader::{ElfError};
use crate::syscalls::synos_syscalls::{SynOSSyscallHandler, SyscallArgs};

// Proper memory layout structure for ELF loading
#[derive(Debug, Clone)]
...
impl ProcessMemoryLayout {
    pub fn from_elf(_elf_data: &[u8]) -> ElfResult<Self> {
        // TODO: Implement ELF parsing
        Ok(Self {
            entry_point: 0x400000,  // Default ELF entry point
            segments: Vec::new(),
            stack_bottom: 0x7fff_0000_0000,
...
impl VirtualMemoryManager {
    ...
    pub fn setup_process_memory(&mut self, layout: &ProcessMemoryLayout) -> Result<(), &'static str> {
        ...
        for segment in &layout.segments {
            ...
            let end_page = (segment.virt_addr + segment.size as u64 + 0xFFF) & !0xFFF;

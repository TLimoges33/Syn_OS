...
use crate::hal::{
    PciManager, GpuDetector, NpuDetector, TpuDetector,
    GpuCapabilities, AiAccelerator, AcceleratorType, HalError
};
...
impl AiAcceleratorRegistry {
    ...
    fn determine_gpu_strategy(&self) -> ExecutionStrategy {
        match self.gpus.len() {
            ...
            _ => {
                // Multiple GPUs
                let gpu_ids: Vec<u32> = self.gpus.iter()
    ...
    pub fn allocate_accelerator(&mut self, process_id: u64, preferred_type: AcceleratorType) -> Option<u32> {
    ...
    pub fn print_accelerator_summary(&self) {
        ...
        crate::println!("   Total Devices: {} GPU(s), {} NPU(s), {} TPU(s)",
            self.gpus.len(),
            self.npus.len(),
            self.tpus.len());

        if !self.gpus.is_empty() {
            let total_vram: u64 = self.gpus.iter()
                .map(|gpu| gpu.capabilities.memory_mb)
                .sum();
